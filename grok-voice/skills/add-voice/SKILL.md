...
Working duplex path: user-app mic in, audio out, `wss://api.x.ai/v1/realtime?model=grok-voice-latest`, safe auth. The coding agent's editor has no native mic; wire the **app** (or a sample client), not the editor.
