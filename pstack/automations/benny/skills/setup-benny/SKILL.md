...
The human enters setup by pointing an agent at the pack's `FOR_AGENTS.md`. The bootstrap flow copies the whole pack into the target repository, then reads this file directly at `.agents/automations/benny/skills/setup-benny/SKILL.md`.

Benny needs external configuration and two live agent automations: triggered agent runs on an automation platform that can read and post in Slack.
...
Do this before asking for Benny configuration and before starting your automation platform's creation flow.
...
Enable pstack in the target repository's committed project-level agent settings, the file your agent reads to enable plugins or skills per project. Create the file if it does not exist.

Preserve every unrelated setting and every other plugin entry. If a pstack entry already exists, change only whether it is enabled. Preserve the file's existing format, including comments. Validate the file after editing it.
...
Tell the user that the project-level agent settings, `.agents/automations/benny/`, and any referenced secret-free configuration must be committed before either automation is enabled. Do not commit them unless the user asks.
...
Use only model slugs shown as available in the automation platform's supported model list. Do not guess a slug and do not carry over a private default.
...
Prefer the platform's configured Slack actions for reads and posts. The optional `BENNY_SLACK_BOT_TOKEN` may fill a narrow gap such as editing one operations status message or downloading an attachment. Store the value in a secret manager or environment, not in YAML.
...
5. Use your automation platform's reviewed creation flow, where a human reviews and saves the automation.
6. Resolve the Slack channels, the repository, and connected integrations the platform offers.
7. Confirm that the copied pack and any referenced configuration files are committed in the same repository where the automation will run.
8. Show the user the draft, obtain approval, confirm readiness, and hand off for the human to save it.
9. Finish that handoff for this automation before starting the next one.
...
After the triage handoff is complete, give the creation flow this complete repro and fix intent:
...
Do not use the creation flow to search for, inspect, or update existing automations.
...
Ask the user to update each existing automation directly in the automation platform's editor. Do not create replacements or duplicates.

### Creation boundary

Never call a direct automation backend service or backend automation tool. Never use a browser URL that carries draft fields. Never build or open a platform deep link. For new automations, the only finish path is the platform's reviewed creation flow, where a human saves the automation.
