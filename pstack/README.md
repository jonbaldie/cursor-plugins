...
**pstack works with any agent and any model.** every frontier model has its strengths and weaknesses. use whichever agent and models you have with pstack. in fact, many of my skills use multi-model workflows to take advantage of each model's unique strengths.

fork it. improve it. make it yours. PRs are welcome! 

## install

install pstack with your agent's plugin or skill installer, or copy `skills/` and `agents/` into the directories your agent loads.
...
[`/poteto-mode`](./skills/poteto-mode/SKILL.md) works extremely well with your agent's loop or scheduled-wake mechanism. you can make your agent work for many hours without sacrificing rigor.
...
pstack also ships a subagent that runs my style end to end. spawn it from a parent agent as [the `poteto-agent` subagent](./agents/poteto-agent.md). it reads `poteto-mode` in full, including its inline principles index, before doing any work. substituting a general-purpose subagent skips that read and drifts.

[`/poteto-mode`](./skills/poteto-mode/SKILL.md) and [the `poteto-agent` subagent](./agents/poteto-agent.md) route through the same wrapper.

pstack also ships [Comment Sicko](./agents/comment-sicko.md), a read-only comment reviewer available as the `Comment Sicko` subagent. usually invoke it through [`/no-comments`](./skills/no-comments/SKILL.md), not directly.
...
a few things `poteto-mode` references but doesn't bundle. pstack expects your agent to supply them, or you ask for the outcome in plain words:

- a deslop pass: remove narrating comments, unsupported guards, dead compatibility paths, and unrelated edits.
- a skill or tool that drives the real app: the CLI or TUI, or the browser or Electron UI.
- your agent's skill-authoring skill, if it has one.
- your agent's loop or scheduled-wake mechanism, for long runs.

if your agent ships its own PR-monitoring command, the [babysit playbook](./skills/poteto-mode/playbooks/babysit.md) supersedes it inside `poteto-mode` for pr-status requests.

## why are there no planning skills?

most agents have a plan mode, and it works great with pstack. but personally, i don't believe in planning. the best spec is code. if you do want to make a plan, [`/poteto-mode`](./skills/poteto-mode/SKILL.md) covers it, but it's not a default. 
...
models are configurable too. type [`/setup-pstack`](./skills/setup-pstack/SKILL.md). it detects the models you have access to and writes a small config file at `~/.config/pstack/models.md` mapping each role (code, judgment, the review panels) to a model. every skill reads it and falls back to sensible defaults when the file is absent, so you override only what you want.
