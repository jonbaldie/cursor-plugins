...
- [ ] Arm the 30-minute audit tick. In a local session, your agent's loop or scheduled-wake mechanism. In a cloud root, a cloud-sleeper wake chain. Never leave the cadence to memory.
...
- [ ] Run a deslop pass (remove narrating comments, unsupported guards, dead compatibility paths, unrelated edits) before each commit and `/no-comments` before review.
...
Each live lane runs on its own cloud VM at the PR head. Drive through a control skill or tool for the UI or CLI/TUI.
...
**Verify, live.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked. Ten lanes on Grok 4.6 Fast (xhigh reasoning) at the PR head, per the boot recipe.
