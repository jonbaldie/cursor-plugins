...
Your chat history is the active workspace's session transcripts, wherever your agent stores them (check the system prompt or the agent's docs; most write one JSONL file per session, one message per line). Find that directory before step 3.
