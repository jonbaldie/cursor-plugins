...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript from the active workspace's session transcripts, wherever your agent stores them (the system prompt or agent docs name the path). Don't glob across other workspaces' transcripts. That reads unrelated private chats. Walk the log against what actually happened:
