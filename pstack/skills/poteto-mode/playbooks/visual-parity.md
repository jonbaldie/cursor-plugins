...
4. Verify each component against its baseline via image diff on the matching surface via the control skill. A nonzero diff is a fail. Investigate the pixel delta. Loop per component, using your agent's loop or scheduled-wake mechanism, until the diff is zero.
