...
Use this reference when the Babysit playbook (`../playbooks/babysit.md`) handles Bugbot or other review-bot and review-automation comments. The rubric applies to any review bot. Bugbot is the worked example. The goal is not to ignore Bugbot by default. The goal is to stop treating every comment as a required code change.
